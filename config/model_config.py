...
class MultiGeomGPTConfig:
    ...
    n_layers: int = 9 # must be n_euc_layers+n_hyp_layers+n_sph_layers in case of multi_geom_block == False, and arbitrary otherwise
    ...
    n_euc_layers: int = 3 # number of fully euc transformer layers (relevant only when multi_geom_block == False)
    n_hyp_layers: int = 3 # number of fully hyp transformer layers (relevant only when multi_geom_block == False)
    n_sph_layers: int = 3 # number of fully sph transformer layers (relevant only when multi_geom_block == False)
